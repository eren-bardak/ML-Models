...
def load_docs(docs_dirname:str) -> Sequence[Sequence]:
    ...
    docs = []
    # YOUR CODE HERE
    lst_of_files = filelist(docs_dirname)
...
def vectorize(V:dict, docwords:Sequence) -> np.ndarray:
    ...
    """
    # YOUR CODE HERE
    vector = np.array([0 for i in range(len(V))])
...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        """
        # YOUR CODE HERE
        vocab_size = len(X[0]) - 1 #total unique words
        w_count_neg_vector = X[(y==0)].sum(axis=0) # a word count vector for negatives
        w_count_pos_vector = X[(y==1)].sum(axis=0) # a word count vector for positives
        w_counts_neg = X[(y==0)].sum(axis=1).sum(axis=0) # total negative words
        w_counts_pos = X[(y==1)].sum(axis=1).sum(axis=0) # total positive words
        self.p_w_c_neg = (w_count_neg_vector + 1) / (w_counts_neg + vocab_size + 1) # P(W|C=neg)
        self.p_w_c_pos = (w_count_pos_vector + 1) / (w_counts_pos + vocab_size + 1) # P(W|C=pos)
        self.p_neg = len(X[(y==0)])/ len(X) # P(c=neg)
        self.p_pos = len(X[(y==1)])/ len(X) # P(c=pos)
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        for k, doc in enumerate(X):
            prob_pos = np.log(self.p_pos)
            prob_neg = np.log(self.p_neg)
            for w in range(len(doc)):
                if doc[w] > 0:
                    prob_pos = (prob_pos) + np.log(self.p_w_c_pos[w]) * doc[w]
                    prob_neg = (prob_neg) + np.log(self.p_w_c_neg[w]) * doc[w]      
...
def kfold_CV(model, X:np.ndarray, y:np.ndarray, k=4) -> np.ndarray:
    ...
    """
    kf = KFold(n_splits=k, shuffle=True, random_state=999) #(#fill this in# ) # use same split every time for student eval
    # YOUR CODE HERE
